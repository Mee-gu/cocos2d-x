...
void GL::viewport(GLint x, GLint y, GLsizei width, GLsizei height)
{
    ...
    _defaultGLState.viewport.x = x;
    _defaultGLState.viewport.y = y;
    _defaultGLState.viewport.w = (unsigned int)width;
    _defaultGLState.viewport.h = (unsigned int)height;
}
...
{
    if (program == _defaultGLState.program)
        RETURN();

    glUseProgram(program);
    _defaultGLState.program = program;
}
